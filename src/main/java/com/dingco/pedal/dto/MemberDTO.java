...
public class MemberDTO {
    ...
    @NotBlank(message = "이메일(아이디)은 필수 입력 값입니다.<br>")
    String email1;
    @Pattern(regexp = "^[a-zA-z0-9.].{1,19}$", message = "이메일을 정확히 입력해주세요.")
    @NotBlank(message = "이메일(주소)은 필수 입력 값입니다.")
    String email2;
    String phone1;
    @NotBlank(message = "연락처(중간 자리)는 필수 입력 값입니다.<br>")
    @Pattern(regexp = "^[0-9].{2,3}$", message = "연락처의 중간 자리는 숫자 3~4자리여야 합니다.<br>")
    String phone2;
    @NotBlank(message = "연락처(마지막 자리)는 필수 입력 값입니다.")

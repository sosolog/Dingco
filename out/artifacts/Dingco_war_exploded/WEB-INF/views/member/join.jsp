...
<h2>회원가입 페이지</h2>
<br>
<form action="join.action" id="joinForm" name="joinForm" method="post" enctype="multipart/form-data" onsubmit="return joinSubmitCheck()">
    ...
    <c:if test="${emailCheckHidden == true}">
        <input type="hidden" value="true" id="emailCheckHidden" name="emailCheckHidden">
    </c:if>
    <c:if test="${emailCheckHidden == false}">
        <input type="hidden" value="false" id="emailCheckHidden" name="emailCheckHidden">
    </c:if>
    <c:if test="${emailCheckHidden == null}">
        <input type="hidden" value="false" id="emailCheckHidden" name="emailCheckHidden">
    </c:if>
    ...
    * 비밀번호 확인:<input type="password" id="passwd1" name="passwd1" onkeyup="memberPwDuplicateCheck()"><br>
    ...
    * 이메일:<input type="text" name="email1" id="email1" value="${memberDTO.email1}">@
    ...
    <br>
    <input type="text" placeholder="인증번호를 입력하세요" id="emailValidationCheckNumber" name="emailValidationCheckNumber" value="${emailValidationCheckNumber}"/>
    <c:if test="${emailCheckHidden == true}">
